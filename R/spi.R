...
spi <- function(fecha, precipitacion, escalas, referencia = spi_referencia(fecha, precipitacion),
  ...
  data <- data.table::as.data.table(completar_serie(data.frame(fecha = fecha, pp = precipitacion),
                                                    fecha, "1 mes"))
  data[, as.character(escalas) := data.table::frollmean(pp, escalas)]
  data[, pp := NULL]
  data <- data.table::melt(data, id.vars = "fecha", value.name = "pp", variable.name = "escala")
  ...
  referencia[, as.character(escalas) := data.table::frollmean(pp, escalas)]
  referencia[, pp := NULL]
  referencia <- data.table::melt(referencia, id.vars = "fecha", value.name = "pp", variable.name = "escala")
  ...
  params <- referencia[, .(params = .(spi_params(pp, scale = 1, na.rm = TRUE,
                                                 distribucion = distribucion, ...))),
                     by = .(escala, month = data.table::month(fecha))]

  # Calcula el SPI usando los parámetros.
  data[, spi := spi_core(stats::ts(pp, frequency = 1), scale = 1, na.rm = TRUE,
                         distribucion = distribucion,
...
spi_referencia <- function(fecha, precipitacion) {
  data.frame(fecha = fecha, pp = precipitacion)
}
